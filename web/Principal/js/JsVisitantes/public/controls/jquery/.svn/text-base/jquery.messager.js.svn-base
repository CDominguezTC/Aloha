 ...
 */
	...
	this.title = "信息提示";//gettext('信息提示');国际化
	this.time = 4000;//默认显示4s--Darcy
	this.anims = {'type' : 'slide', 'speed' : 600};
    this.immediate = false;
	this.inits = function(title, text){
		if($("#message").is("div")){ return; }
		$(document.body).prepend('<div id="message" style="border:#b9c9ef 1px solid;z-index:100;width:'+this.layer.width+'px;height:'+this.layer.height+'px;position:absolute; display:none;background:#cfdef4; bottom:2px; right:8px;">'
                                    ...
                                    +'</div>'
                                    +'<div id="message_down" style="padding-bottom:5px;border:1px solid #fff;border-top:none;width:99%;height:150px;font-size:12px;">'
                                        +'<div id="message_content" style="margin:0 5px 0 5px;border:#b9c9ef 1px solid;padding:5px 0 5px 5px;font-size:12px;width:'+(this.layer.width-20)+'px;height:'+(this.layer.height-85)+'px;color:#1f336b;text-align:left;">'+text+'</div>'
                                    +'</div>'
                                +'</div>');
	};
